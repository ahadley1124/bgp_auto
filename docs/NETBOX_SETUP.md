...
This guide explains what needs to be created in NetBox for each router to work with `bgp_auto`.

## Prerequisites

- NetBox instance running and accessible
- Admin or API token with read/write permissions
- API base URL, for example `https://netbox.example.com`
- Organization and site already created in NetBox
...
| **Type**         | Text                                            |
| **Required**     | No                                              |
| **Description**  | WireGuard private key generated by the playbook |
...
| **Type**         | Text                                           |
| **Required**     | No                                             |
| **Description**  | WireGuard public key generated by the playbook |
...
| **Required**     | Yes (if WireGuard is enabled)                                          |
| **Description**  | JSON list of peer device names, for example `["router02", "router03"]` |

The playbook reads the `peers` custom field first. For backwards compatibility, it falls back to the legacy `wg_peers` field if `peers` is not present.

### Field 6: `ospf_enabled`
...
- **Input:** `loopback_ip`, GRE prefixes tagged with the NetBox prefix role `gre-tunnels`
- **Behavior:** Tunnel internal IP addresses are discovered from NetBox prefixes with role `gre-tunnels` and allocated from the next available `/31` pairs using NetBox's `available-ips` API. If the tunnel IPs do not already exist in NetBox, the playbook requests new IPs during deployment and uses them for the GRE interfaces.
...
Create a device role named **Router** for device filtering.
...
Create a device type, for example **Generic Router**, to assign to routers.
...
Create a prefix for loopback addresses, for example `10.0.0.0/24`.
...
Repeat for each router with the appropriate loopback IPs.
...
2. Verify the inventory plugin discovers devices:
...
Check that `wg_private_key` and `wg_public_key` are populated.
...
### GRE Reference (Optional)

- **Input:** `loopback_ip`
- **Output:** Tunnel IPs allocated from NetBox prefix role `gre-tunnels`
- **Generates:** GRE tunnel config
...
**Problem:** `ansible-inventory` returns an empty list.

**Solution:**

1. Verify device role is exactly `Router` with slug `router`.
2. Check device status is `Active`.
3. Run `curl "$NETBOX_API/api/dcim/devices/?role=router"`.

### Issue: Custom fields appear as `None`

**Problem:** `netbox_cf_loopback_ip` shows `None` in the playbook.

**Solution:**

1. Verify the custom field exists in Administration → Custom Fields.
2. Verify the custom field is assigned to Content Type: DCIM / Device.
3. Verify the value is entered on the device page.
4. Clear inventory cache with `rm -rf /tmp/ansible_netbox_inv_*`.

### Issue: WireGuard keys not syncing to NetBox

**Problem:** First playbook run fails or keys are not stored.
...
**Problem:** BIRD complains about a missing loopback IP.

**Solution:**

1. Verify interface `lo` exists on the device.
2. Verify IP address `X.X.X.X/32` is assigned to the `lo` interface.
3. Check the IP status is `Active`.

## NetBox Backup

Regularly back up your NetBox database to preserve peer configuration.
...
3. Test inventory discovery with `ansible-inventory -i inventory/netbox.yml --list`.
4. Run the initial deployment with `ansible-playbook -i inventory/netbox.yml playbooks/deploy.yml`.
5. Verify BGP peering with `ssh router01 birdc show protocols`.
